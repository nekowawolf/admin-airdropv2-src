...
export default function GithubReposTable({
  data,
  loading,
  error,
  onDelete,
  editRoute = '/github-repos-menu/dashboard/edit-github-repo',
  title = 'GitHub Repos List',
  subtitle = 'Manage your GitHub Repos listings'
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success("GitHub Repo deleted successfully!")
    } catch (err: any) {
      toast.error(err.message || "Failed to delete GitHub Repo.")
    } finally {
        ...
        createPortal(
          <div className="fixed inset-0 flex items-center justify-center bg-[var(--overlay-bg)] z-50">
            <div className="dropdown-bg rounded-lg shadow-lg p-6 max-w-sm w-full text-center">
              ...
              <h3 className="text-primary text-lg font-semibold mb-2">Delete GitHub Repo</h3>
              <p className="text-secondary mb-6">
                Are you sure you want to delete this GitHub Repo:{" "}
      ...
      {!loading && !error && totalPages > 1 && (
        <>
          ...
          <div className="text-center text-xs text-muted-foreground mt-2 text-secondary">
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredData.length)} of {filteredData.length} GitHub Repos
          </div>
        </>
      )}
    </div>
