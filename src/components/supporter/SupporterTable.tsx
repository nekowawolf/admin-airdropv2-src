...
export default function SupporterTable({
  ...
  error,
  onDelete,
  editRoute = '/supporter-menu/dashboard/edit-supporter',
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.error(err.message || "Failed to delete Supporter.")
    } finally {
          ...
          <table className="w-full text-left">
            <thead className="bg-[var(--card-color3)]">
              <tr>
                ...
                <th className="px-6 py-2 min-w-[120px]">Amount</th>
            ...
            <tbody>
              {paginatedData.length > 0 ? (
                ...
                <tr>
                  <td colSpan={5} className="text-center py-4 text-secondary">No Supporters found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        createPortal(
          <div className="fixed inset-0 flex items-center justify-center bg-[var(--overlay-bg)] z-50">
            <div className="dropdown-bg rounded-lg shadow-lg p-6 max-w-sm w-full text-center">
              ...
              <h3 className="text-primary text-lg font-semibold mb-2">Delete Supporter</h3>
      ...
      {!loading && !error && totalPages > 1 && (
        <>
          ...
          <div className="text-center text-xs text-muted-foreground mt-2 text-secondary">
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredData.length)} of {filteredData.length} Supporters
          </div>
        </>
      )}
    </div>
