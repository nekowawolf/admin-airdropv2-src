...
export default function CreatorsTable({
  ...
  error,
  onDelete,
  editRoute = '/creators-menu/dashboard/edit-creator',
  title = 'Creators List',
  subtitle = 'Manage your Creators listings'
}: CreatorsTableProps) {
              ...
              {paginatedData.length > 0 ? (
                paginatedData.map((item, index) => (
                  <tr key={index} className="border-t border-border-divider">
                    ...
                    <td className="px-6 py-2 whitespace-nowrap">{item.language || 'N/A'}</td>
                    <td className="px-6 py-2">
                      {item.open_to_work ? (
        ...
        createPortal(
          <div className="fixed inset-0 flex items-center justify-center bg-[var(--overlay-bg)] z-50">
            <div className="dropdown-bg rounded-lg shadow-lg p-6 max-w-sm w-full text-center">
              ...
              <h3 className="text-primary text-lg font-semibold mb-2">Delete Creator</h3>
      ...
      {!loading && !error && totalPages > 1 && (
        <>
          ...
          <div className="text-center text-xs text-muted-foreground mt-2 text-secondary">
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredData.length)} of {filteredData.length} Creators
          </div>
        </>
      )}
    </div>
