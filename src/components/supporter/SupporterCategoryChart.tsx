...
export default function SupporterCategoryChart({ data }: { data: SupporterResponse[] }) {
  const categoryCount = data.reduce((acc: any, curr: any) => {
    if (curr.platform) {
      const trimmed = curr.platform.trim()
  ...
  return (
    <div className="p-4 rounded-2xl shadow-md bg-[var(--fill-color)] border border-border-divider h-80">
      <h3 className="text-lg font-semibold text-primary mb-4">Supporters by Platform</h3>
