...
interface RepoSubmissionsTableProps {
  data: RepoSubmission[]
  loading: boolean
  error: string | null
...
export default function RepoSubmissionsTable({
  data,
  loading,
  error,
  onDelete,
  title = 'Repo Submissions',
  subtitle = 'Review and manage incoming repository submissions'
}: RepoSubmissionsTableProps) {
  ...
  const filteredData = useMemo(() => {
    return safeData.filter(item =>
      (item.repo_url?.toLowerCase() || '').includes(search.toLowerCase()) ||
        ...
        <div className="overflow-x-auto rounded-lg bg-[var(--fill-color)] border border-border-divider">
          <table className="w-full text-left">
            <thead className="bg-[var(--card-color3)]">
              <tr>
                <th className="px-6 py-2 min-w-[200px]">Repo URL</th>
                  ...
                  <tr key={index} className="border-t border-border-divider">
                    <td className="px-6 py-2">
                      {item.repo_url ? (
                        ...
                        >
                          {item.repo_url.length > 40 ? item.repo_url.slice(0, 40) + '...' : item.repo_url}
                        </a>
                      ) : (
                        'N/A'
                      )}
