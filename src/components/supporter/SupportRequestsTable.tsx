...
export default function SupportRequestsTable({
  ...
  subtitle = 'Manage your Support Requests submissions'
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.error(err.message || "Failed to delete Support Request.")
    } finally {
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {paginatedData.length > 0 ? (
                ...
                <tr>
                  <td colSpan={5} className="text-center py-4 text-secondary">No Support Requests found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        createPortal(
          <div className="fixed inset-0 flex items-center justify-center bg-[var(--overlay-bg)] z-50">
            <div className="dropdown-bg rounded-lg shadow-lg p-6 max-w-sm w-full text-center">
              ...
              <h3 className="text-primary text-lg font-semibold mb-2">Delete Support Request</h3>
      ...
      {!loading && !error && totalPages > 1 && (
        <>
          ...
          <div className="text-center text-xs text-muted-foreground mt-2 text-secondary">
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredData.length)} of {filteredData.length} Support Requests
          </div>
        </>
      )}
    </div>
