...
export default function AIToolsTable({
  ...
  error,
  onDelete,
  editRoute = '/ai-tools-menu/dashboard/edit-ai-tool',
  title = 'AI Tools List',
  subtitle = 'Manage your AI Tools listings'
}: AIToolsTableProps) {
        ...
        createPortal(
          <div className="fixed inset-0 flex items-center justify-center bg-[var(--overlay-bg)] z-50">
            <div className="dropdown-bg rounded-lg shadow-lg p-6 max-w-sm w-full text-center">
              ...
              <h3 className="text-primary text-lg font-semibold mb-2">Delete AI Tool</h3>
              <p className="text-secondary mb-6">
                Are you sure you want to delete this AI Tool:{" "}
      ...
      {!loading && !error && totalPages > 1 && (
        <>
          ...
          <div className="text-center text-xs text-muted-foreground mt-2 text-secondary">
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredData.length)} of {filteredData.length} AI Tools
          </div>
        </>
      )}
    </div>
