...
export default function LinkPostTable({
  data,
  loading,
  error,
  onDelete,
  ...
  const handleEdit = (item: LinkPost) => {
    router.push(`${editRoute}/${item._id}`)
    setOpenDropdownIndex(null)
  }

  const handleDeleteClick = (id: string, caption: string) => {
    setSelectedId(id)
          ...
          <table className="w-full text-left min-w-[900px]">
            ...
            <tbody>
              {paginatedData.length > 0 ? (
                      ...
                      <td className="px-6 py-2">
                        {item.caption ? (
                          <div className="max-w-[280px] truncate">
                            {item.caption.length > 20 ? item.caption.slice(0, 20) + '...' : item.caption}
                          </div>
                        ) : (
                          <div className="max-w-[280px] text-muted">N/A</div>
                        )}
                      </td>
                      <td className="px-6 py-2">
                ...
                <tr>
                  <td colSpan={5} className="text-center py-4 text-secondary">No posts found.</td>
                </tr>
              )}
            </tbody>
          </table>
              ...
              <li>
                <button
                  onClick={() => handleDeleteClick(
                    ...
                    paginatedData[openDropdownIndex].caption || 'Unknown'
                  )}
        ...
        createPortal(
          <div className="fixed inset-0 flex items-center justify-center bg-[var(--overlay-bg)] z-50">
            <div className="dropdown-bg rounded-lg shadow-lg p-6 max-w-sm w-full text-center">
              ...
              <h3 className="text-primary text-lg font-semibold mb-2">Delete Post</h3>
              <p className="text-secondary mb-6">
                Are you sure you want to delete this post:{" "}
                <span className="font-semibold text-primary">{selectedCaption?.slice(0, 50)}</span>?
              </p>
      ...
      {!loading && !error && totalPages > 1 && (
        <>
          ...
          <div className="text-center text-xs text-muted-foreground mt-2 text-secondary">
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredData.length)} of {filteredData.length} posts
          </div>
        </>
      )}
    </div>
